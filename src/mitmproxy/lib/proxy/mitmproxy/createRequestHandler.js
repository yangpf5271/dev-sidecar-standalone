...
module.exports = function createRequestHandler (createIntercepts, middlewares, externalProxy, dnsConfig, setting, compatibleConfig) {
  ...
  return function requestHandler (req, res, ssl) {
    ...
    // 自代理守卫：目标是自己监听的地址时直接拒绝，避免请求死循环
    // （如 Docker healthcheck 直接 GET 代理自身端口）
    const selfPort = req.socket.localPort
    if (selfPort != null && rOptions.port === selfPort &&
        (rOptions.hostname === '127.0.0.1' || rOptions.hostname === 'localhost' || rOptions.hostname === req.socket.localAddress)) {
      res.writeHead(400, { 'Content-Type': 'text/plain; charset=utf-8' })
      res.end('dev-sidecar-standalone: refusing to proxy request to itself')
      return
    }
    ...
    function countSlow (isDnsIntercept, reason) {
      if (isDnsIntercept && isDnsIntercept.dns && isDnsIntercept.ip !== isDnsIntercept.hostname) {
        ...
        // 同时从 speed tester 的 alive 列表中移除坏 IP，使下次请求切换到其他 IP
        const tester = speedTest.getSpeedTester(hostname, rOptions.port)
        if (tester && tester.alive.length > 0) {
          const before = tester.alive.length
          tester.alive = tester.alive.filter(item => item.host !== ip)
          if (tester.alive.length < before) {
            log.info(`已从 alive 列表移除坏 IP: ${ip}, 剩余 ${tester.alive.length} 个, 下次请求将使用: ${tester.alive[0]?.host || '无'}`)
          }
        }
      }
    ...
    (async () => {
        ...
        try {
          if (resIncpts && resIncpts.length > 0) {
            for (const resIncpt of resIncpts) {
              resIncpt.responseIntercept(context, req, res, proxyReq, proxyRes, ssl, next)
              // 判断是否已经关闭
              if (res.writableEnded) {
                next()
                return
      ...
      if (!res.writableEnded) {
        try {
          ...
          // HTML 转义，防止错误信息中的请求可控内容（主机名/路径）注入页面
          ...
          <p>目标网站请求错误：【${escapeHtml(e.code)}】 ${escapeHtml(e.message)}</p>
          <p>目标地址：${escapeHtml(`${rOptions.protocol}//${rOptions.hostname}:${rOptions.port}${rOptions.path}`)}</p>`,
          )
        } catch {
          // do nothing
        }
